...
Log.Logger = new LoggerConfiguration()
    ...
    .Enrich.WithProperty("ServiceName", "HotelService")
    .WriteTo.Console(outputTemplate: "[{Timestamp:HH:mm:ss} {Level:u3}] {ServiceName} | {Message:lj}{NewLine}{Exception}")
    .WriteTo.File("Logs/hotelservice-.log",
...
try
{
    Log.Information("Starting Hotel Service...");
    ...
    {
        c.SwaggerDoc("v1", new OpenApiInfo { Title = "Hotel Service API", Version = "v1" });
...
{
    Log.Fatal(ex, "Hotel Service failed to start");
}
finally
{
    Log.CloseAndFlush();
